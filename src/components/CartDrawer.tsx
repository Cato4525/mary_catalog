...
export default function CartDrawer({ open, onClose }: Props) {
  ...
  const plainMessage = (() => {
    ...
    return `🛍️ *Nuevo Pedido - Mary Leggings*\n\n${parts.join("\n\n")}\n\nTotal: ${total} producto(s)`
  })()
  ...
  async function handleSendOrder() {
    ...
    if (!canNativeShare) {
      window.open(`https://wa.me/${whatsapp}?text=${encodeURIComponent(plainMessage)}`, "_blank", "noopener,noreferrer")
      return
    }
      ...
      await Promise.all(
        items.map(async (item) => {
          ...
          try {
            ...
            files.push(new File([blob], `producto-${item.id}.${ext}`, { type }))
          } catch {}
        })
      )

      if (files.length > 0 && navigator.canShare({ files, text: plainMessage })) {
        await navigator.share({ files, text: plainMessage })
        return
      }
    } catch (err) {
            ...
            <div className="border-t border-gray-200 px-4 py-4">
              {whatsapp ? (
                <button
                  ...
                  disabled={sending}
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-green-600 px-6 py-3 text-sm font-medium text-white shadow-sm transition-all hover:bg-green-700 hover:shadow-md active:scale-[0.97] disabled:opacity-60"
                >
                  ...
                  {sending ? "Preparando imágenes..." : "Enviar pedido por WhatsApp"}
                </button>
              ) : (
